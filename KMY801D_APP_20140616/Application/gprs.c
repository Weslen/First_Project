...
void kmy_GprsOnOff(uint8_t OnOff)
{
	if(OnOff)	//4.2V and GPRSMoudle on
	{
		GPIO_SetBits (GPIOC, PinGprsKey);
		msleep(2000);
		GPIO_ResetBits (GPIOC, PinGprsKey);
	}
	else
	{
		//4.2V and GPRSMoudle off
		GPIO_ResetBits (GPIOC, PinGprsKey | PinGprsPWR);
	}
}
...
void kmy_GprsInit(void)
{
	...
	/* Configure PB.10~11 pin as output PP */
	GPIO_InitStructure.GPIO_Pin = PinGprsKey | PinGprsPWR;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init (GPIOC, &GPIO_InitStructure);

	//4.2V and GPRSMoudle ON
	GPIO_SetBits (GPIOC,PinGprsPWR);
	GPIO_ResetBits (GPIOC, PinGprsKey);
	kmy_GprsOnOff(1);
	kmy_USART2Init(115200);
}
...
static unsigned char _tcp_Connect(unsigned char *DomainOrIP)
{
	...
	{
		for(i=0;i<25;i++){if(*DomainOrIP!=','){serverip[i]=*DomainOrIP;DomainOrIP++;}}
		DomainOrIP++;
		for(i=0;i<5;i++){if(*DomainOrIP>='0'&&*DomainOrIP<='9'){serverport[i]=*DomainOrIP;DomainOrIP++;}}
	}
