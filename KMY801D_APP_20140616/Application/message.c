...
static void DisplayMessage(char *msg)
{
	...
	my_itoa(&atcmd[8],*msg);
	strcat(atcmd,"\r");
	send_at_cmd((const unsigned char*)atcmd);
...
void GetMessageTotalAndCapacity(unsigned char *Total,unsigned char *Capacity)
{
	...
	*Capacity=*Total=0;
	...
	if(send_at_cmd_and_receive(buff,sizeof(buff),200,"OK")==TCP_OK){
		p=strstr((const char*)buff,"+CPMS:");
		if(p==NULL)return;
		p=strstr((const char*)p,",");if(p==NULL)return;
		p++;
		if(Total!=NULL)*Total=atoi((const char*)p);

		p=strstr((const char*)p,",");if(p==NULL)return;
		p++;
		if(Capacity!=NULL)*Capacity=atoi((const char*)p);
	}
}
...
void GetMessageNode(struct sMessageNode *p_msgNode,unsigned char Total,unsigned char Capacity,unsigned char readed)
{
	...
	for(i=0;i<Capacity;i++){
		...
		strcat((char *)buff,(const char*)temp);
...
char MessageDisplayContext(struct sMessageNode *pm)
{
	...
	p=(unsigned char*)strstr((const char*)buff,"CMGR");
	if(p==NULL)return 0;
	p=(unsigned char*)strstr((const char*)p,"\x0d\x0a");
	if(p==NULL)return 0;
	p+=2;

	p2=(unsigned char*)strstr((const char*)p,"\x0d\x0a");
	if(p2==NULL)return 0;

	my_buff_TwoAscciiToOneHex(p,(unsigned int)(p2-p));
	ucs2str_to_gb2312str(p,(unsigned int)(p2-p)/2,buff);
...
char ReceiveMailBox (char *msg) //收件箱
{
	...
	while (1)
	{
		...
		else if ( (op.flashflag & FlushAllFlag) != 0)
		{
			...
			{
				if (Msg[op.Menucnt - op.curFlashRow + i].readflag == MESSAGE_UNREAD)
				{
					steaddraw_pic_to_row_x (MessageIco_unread, 0, i, 12, 12);
				}
				else
				{
					steaddraw_pic_to_row_x (MessageIco_readed, 0, i, 12, 12);
				}
...
static signed char CheckPINAndPUK(void)
{
	...
	if(strstr((const char*)CmdBuff,"SIM PIN")){
		RequirePin=0x01;
